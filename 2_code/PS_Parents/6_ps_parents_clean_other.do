...
di as txt "======================================================="
di as txt "STARTING: PS Parents Clean Other"
di as txt "======================================================="
...
capture confirm file "${processed_data}/PS_Parents/5_ps_parents.dta"
if _rc {
    di as error "ERROR: Input file not found: ${processed_data}/PS_Parents/5_ps_parents.dta"
    di as error "Run 5_ps_parents_clean_motivations.do first."
    exit 601
}
...
di as txt "Identifying apprenticeship plan variable..."
local planvar ""
local plan_found = 0
...
if "`planvars'" == "" {
    di as txt "No variable matching 'plan*' found. Checking for other candidates..."
    
    // Try other common apprenticeship variable names
    foreach var in apprenticeship apprenticeship_plan child_plan child_apprenticeship future_plan {
        capture confirm variable `var'
        if !_rc {
            di as txt "Found alternative plan variable: `var'."
            local planvar "`var'"
            local plan_found = 1
            continue, break
        }
    ...
    di as txt "Found plan variable: `planvar'."
    local plan_found = 1
}

if `plan_found' == 0 {
    di as error "ERROR: Could not identify a suitable apprenticeship plan variable."
    di as error "Cannot process apprenticeships."
    exit 111
}
else {
    // Standardize variable name to "plan" for consistency
    rename `planvar' plan
}
...
// Final report
di as txt "Cleaned other variables dataset saved to: ${processed_data}/PS_Parents/6_ps_parents.dta"
di as txt "Observations: `=_N'"
di as txt "Variables: `=c(k)'"
di as txt "======================================================="
di as txt "COMPLETED: PS Parents Clean Other"
di as txt "======================================================="
